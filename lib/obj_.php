...
//for new registration. 
    public function newRegEmail($uid, $pwd, $email, $table) {
        $insert = "insert into $table set uid = :uid, pwd = :pwd , email = :email";
        $statement = $this->conn->prepare($insert);
        $statement->execute(array(':uid' => $uid, ':pwd' => $pwd, ':email' => $email));
        return true;
    }
